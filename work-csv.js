  ...
  for(var i = 1; i < 30; i++) {
	if(data.sname[i]) {
	  ...
	  for(var j = 0; j < data.dates_code.length; j++) {
		csv += `"${data.data[data.dates_code[j]].seats ? (data.data[data.dates_code[j]].seats.includes(i) ? "X" : "") : ""}",`;
		csv += `"${data.data[data.dates_code[j]].jobs ? (data.data[data.dates_code[j]].jobs.includes(i) ? "X" : "") : ""}",`;
	  }
	  csv += "\n";
	}
  }
